...
/* Downloads the contents of the pipe to reply[].  
 * Returns: The length of the download +1	*/
int manReplyReceive(managerLink * manLink, char reply[])
{
	int n;
...
void manUploadPacket(managerLink * manLink)
{
	...
	printf("Enter filename in host's directory: ");
	scanf("%s",fname);
...
void manSetMainDir(managerLink * manLink)
{
	...
	/* Create the command message */
	/* Initialize command to the empty string */
	command[0] = '\0'; 
...
/* Tell the host to send its state information.  
 * First, it creates the command message "GetHostState".  
 * Then it sends the message to the host */
void manGetHostState(managerLink * manLink)
{
	char command[1000];
	command[0] = '\0';                         /* Empty command string */
